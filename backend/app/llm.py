"""Shared Gemini structured-call helper, lifted from extraction/engine.py so the
screener plane reuses the exact same retry/validation/tracing contract instead
of growing a second calling convention.

PII rule (unchanged from the engine): log only content hashes, trace names,
and error *shapes* — never prompt text, page images, or model output values.
"""
import asyncio
import logging
...
async def call_gemini(
    ...
    *,
    ...
    document at once) is optional — screener assessment nodes are text-only.
