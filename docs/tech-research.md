...
- If the target form is Yunaki-owned: say so in the design doc and propose a structured submission API; browser automation of your own form is technical debt with an expiration date.
...
| Yunaki (target company) | Immigration platform | SOC 2 Type I → Type II observation; attorney-led, human-in-the-loop | Review lane is a feature, not a compromise |
