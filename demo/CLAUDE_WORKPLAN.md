...
2. **Case handoff listener.** Watches `#cases` (SLACK_CHANNEL_CASES). When the attorney posts a handoff (demo format: free text like *"New marriage case, adjustment of status. Ravi — ravi.kumar.demo@example.com, spouse Mei — mei.lin.demo@example.com"*), parse via one LLM structured-output call → `{process_type, parties[]}` → create case + parties + intakes via /core → emit `case.handoff_received`. **Null over guess:** unparseable field ⇒ null + thread reply asking for it, never invented.
