...
/**
 * Matter detail screen body. Shared by the web dynamic route
 * (`/matters/[id]`) and the desktop static twin (`/matter?id=`) so both render
 * the exact same UI — the only difference is how `matterId` is sourced (route
 * param vs query param), which the two thin page shells resolve.
 */
export function MatterDetailView({ matterId }: Props) {
